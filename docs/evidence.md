...
The later agent searched, opened, and exactly cited `reduce_worker_concurrency` version 1 with content digest `d3a78ad553c4472e75292c107acff8ad8e939a553319cfba9115c004917714ed`, and its proposed capability became correct (0 → 1). Its diagnosis, required evidence, and tool coverage were already correct and stayed correct; it retained the generic health observation, so distractor avoidance did not improve and this run does not establish that retrieval improved its reasoning. The explicit citation improved from 0 to 1 at the cost of one additional tool call and roughly two seconds of latency. The before run's wrong proposed capability and the after run's exact citation are the observable difference the governed capability loop is designed to produce; this is one stochastic live comparison, not statistical significance. The offline visibility regression verifies that unpromoted states are never searchable and that a citation must match a capability actually opened in the same investigation thread.
